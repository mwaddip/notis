...
| `setHeadersHandler(cb)` | `((height: number) => OrderingBlock \| null) => void` | Provider for `GetHeaders` / `GetBlocks` (codes 14, 16), for the block ids `SyncInfo` carries (tip id and anchors), and for the id index an inbound `Inv` or `ModifierRequest` is resolved against. Returns the whole block, not the header: one provider serves both responses — `Headers` reads `.header`, `Blocks` returns the block. **Never the tip height** — that is the row below |
