...
- `protocolVersion` is the era the node reports — `GET /health` → `protocol_version` — never a
  constant of the client's build: the field is in every id preimage, so the client learns the era
  before it signs (`ARCHITECTURE → Protocol Versioning`)
