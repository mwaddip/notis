...
| `settlement` | `buildSettlement` · `checkSettlement` · `contributeToBody` · `emptyBody` · `bondOutputOf` · `settlementMarginalBytes` | `NODE_INTERFACE → The settlement transaction` | `SettlementDeps` |
...
`UtxoEngineDeps`, `UtxoResult`, `SettlementDeps`, `SettlementBody`, `DecayDeps`, `DecayPlan`, `EmbeddedTx`, and the two
shapes below. **The export list is the node's present call graph, not a promise** — a helper the node stops calling
leaves the barrel, and one a leaf needs joins it with its caller.
...
`packages/consensus/test/`**, with the fixture helpers it needs in `test/helpers.ts`. A suite that drives the node's
store or block application — `utxo-engine.test.ts` among them, over an in-memory database — stays in
`packages/node/test/` and imports from the package.
