...
| `GET` | `/vouches?target=X&limit=50&after=<boxId>` | `getVouchesForTargetPage` | `{ vouches: [{ voucherId, voucherName, targetId, targetName }], count, next }` — one page of the identity's vouchers, each name the holder's username as typed or `null` (→ Usernames, "A list row carries its names"), ascending box id, strictly after `after`; `count` over the whole set, `next` the key to continue from (HTTP API → "Every list a view returns is a page") |
| `GET` | `/vouches?voucher=X&limit=50&after=<boxId>` | `getVouchesForVoucherPage` | `{ vouches: [{ boxId, value, voucherId, voucherName, targetId, targetName, createdAtBlock }], count, next }` — one page of the identity's live vouches, the names as on the target arm, ascending box id strictly after `after`; `count` over the whole set, `next` the key to continue from. The one arm carrying `boxId`: the unvouch builder names the box it spends |
...
> ⚠ **AHEAD OF CODE — 2026-09-10:** `voucherName` and `targetName` on both vouch arms. The rows carry the two ids and no name.
...
| `GET` | `/invites/:userId?limit=50&after=<boxId>` | `{ bonds: [{ id, value, inviterId, inviterName, inviteePublicKey, inviteeName, createdAtBlock }], bondCount, next }` — the inviter's **unspent** bonds, each with its box's `createdAtBlock` (the bond's settle height) and each name the holder's username as typed or `null` (→ Usernames, "A list row carries its names"), one page ascending box id strictly after `after`, `bondCount` over the whole set, `next` the key to continue from; a bond IS the open invite, so a settled one is not listed and there is no second list | 400 if `userId` is not 64 chars or a present `limit`/`after` does not parse; an inviter holding no live bond answers `{ bonds: [], bondCount: 0, next: null }` |

> ⚠ **AHEAD OF CODE — 2026-09-10:** `inviterName` and `inviteeName` on the bond rows. The rows carry the two keys and no name.
...
the invite and vouch routes mirror theirs; the 409s are the pool's gates. `/status` carries `usernameCount`
(→ Status).

**A list row carries its names.** Every list row the HTTP API serves that carries an identity carries that
identity's username beside it, as typed, or `null` where it holds none — `authorName` beside `author` on every
`PostJson` and `WithdrawnJson` (→ Posts), `voucherName` and `targetName` on both vouch arms (→ Vouches),
`inviterName` and `inviteeName` on the bond rows (→ UTXO queries) — filled by **one keyed read per distinct
identity per response**, through the one name-fill implementation the feed service and the routers share. The
rule is by the row's shape, not by what a client renders: a name that names the request's own subject on every
row of a page (`targetName` on the target arm, `inviterName` on the bonds) is carried for one read. Not a list
...
> ⚠ **AHEAD OF CODE — 2026-09-10:** the four names on the vouch and bond rows, and the shared helper. The feed
> service fills `authorName` through a private per-response cache; the two routers fill no name.
...
> `getDescendantCount` are the stated exceptions: the CTE enumerates the subtree on
> `dag_parent_refs (parent_id)`, so each is O(subtree) — the count is one walk per row it is read
> for, proportional to the replies under that row, and never a join back to `dag_posts` after the
> walk, the form that scans the table once per row. Nothing else
