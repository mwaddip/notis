...
> ⚠ **AHEAD OF CODE (2026-09-05)** — the `roots=1` filter on `GET /posts`, and `descendantCount` and `authorVouchCount` on `PostJson`.
...
| `GET` | `/invites/:userId?limit=50&after=<boxId>` | `{ bonds: [{ id, value, inviterId, inviteePublicKey, createdAtBlock }], bondCount, next }` — the inviter's **unspent** bonds, each with its box's `createdAtBlock` (the bond's settle height), one page ascending box id strictly after `after`, `bondCount` over the whole set, `next` the key to continue from; a bond IS the open invite, so a settled one is not listed and there is no second list | 400 if `userId` is not 64 chars or a present `limit`/`after` does not parse; an inviter holding no live bond answers `{ bonds: [], bondCount: 0, next: null }` |

> ⚠ **AHEAD OF CODE (2026-09-05)** — `invitedAtBlock` on `/karma/:userId` and `createdAtBlock` on the `/invites/:userId` bond row.
...
> ⚠ **AHEAD OF CODE (2026-09-05)** — `queryPostsPage`'s `roots`, `getDescendantCount`, and the pending window's index in the cost lead below.
...
> ⚠ **AHEAD OF CODE (2026-09-05)** — `getVouchCountForTarget`.
