...
The sprite in `_nav.html` is generated — do not hand-edit the block between the
`MARK-DEFS` markers. It is regenerated by `tmp/logo/inject_mark.py`, which lists
this file as a target. Instances are `<use href="#mark…">`, and the artwork must
stay inline SVG because `--notis-green` and `--notis-keyline` do not cross into
an `<img>`.
