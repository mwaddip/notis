...
export type Mode = { kind: 'workspace' } | { kind: 'standalone'; id: string };

const HEX64 = /^[0-9a-f]{64}$/i;

export function decideMode(pathname: string, base: string): Mode {
  const prefix = base + 'p/';
  if (!pathname.startsWith(prefix)) return { kind: 'workspace' };
  const tail = pathname.slice(prefix.length);
  if (!HEX64.test(tail)) return { kind: 'workspace' };
  return { kind: 'standalone', id: tail.toLowerCase() };
}
