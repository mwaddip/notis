...
// ---------------------------------------------------------------------------
// cumulativeWork — one unaccountable header costs one header, not the total
//
// This sum leaves the node as `SyncInfo.tipCumulativeWork` and is what fork
// choice compares, so the cost of a stored header the method cannot account
// for is the property under test. Skipping is `blockWork`'s domain refusal
// (VALIDATION_INTERFACE → blockWork / cumulativeWork), read here as "this
...
describe('LazySyncStore.cumulativeWork', () => {
  it('totals the work of every stored header', () => {
    ...
    expect(store.cumulativeWork()).toBe(2n * (1n << 10n));
  });

  it('stops at the first gap, exactly as chainHeight does', () => {
    // A store with 1 and 3 but no 2 has height 1, so the third header is
    // outside the walk rather than merely unaccounted for. Without this, a
    ...
    expect(store.cumulativeWork()).toBe(1n << 10n);
  });

  it('is 0 when no headers handler has been registered', () => {
  ...
  it.each([
    ...
    expect(() => store.cumulativeWork()).not.toThrow();
    expect(store.cumulativeWork()).toBe(2n * (1n << 10n));
  });
  ...
  it('skips a negative powTargetBits, by refusal rather than by shift semantics', () => {
    ...
    expect(store.cumulativeWork()).toBe(0n);
  });
});
