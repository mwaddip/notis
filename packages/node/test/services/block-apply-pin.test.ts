 ...
 *
 * **Do not regenerate the fixture.** The capture below writes only when the
 * file is absent and `BLOCK_APPLY_PIN_CAPTURE=1` is set, and never overwrites:
 * a difference from it is a finding about the change under test.
 */
...
describe('block application, pinned', () => {
  beforeAll(async () => {
    ...
    vi.resetModules();
    capture = await runApplyPinScenario();
  }, 120_000);
