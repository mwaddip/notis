...
function postLockTx(boxes: {boxId:string,value:string}[], lockAmount:bigint, targetPostId:string): UtxoTransaction {
  ...
  return {
    ...
    outputs: [
      { boxType:'karma',value:t-lockAmount,owner:pubRaw },
      { boxType:'post_lock',value:lockAmount,originalValue:lockAmount,owner:pubRaw,targetPostId },
    ],
    signatures:{},
...
function likeTx(boxes: {boxId:string,value:string}[], targetPostId: string): UtxoTransaction {
  const t = boxes.reduce((s,b)=>s+BigInt(b.value),0n);
  return { inputs: boxes.map(b=>b.boxId), outputs: [{ boxType:'karma',value:t-LIKE_COST,owner:pubRaw }, { boxType:'like',value:LIKE_COST,likerId:pubRaw,targetPostId }], signatures:{}, protocolVersion:PROTOCOL_VERSION };
}
function inviteTx(boxes: {boxId:string,value:string}[], secretHashHex: string): UtxoTransaction {
  ...
  return { inputs: boxes.map(b=>b.boxId), outputs: [{ boxType:'karma',value:t-s,owner:pubRaw }, { boxType:'invite',value:INVITE_KARMA_AMOUNT,secretHash:unhex(secretHashHex),inviterId:pubRaw }, { boxType:'bond',value:INVITE_BOND_KARMA,inviterId:pubRaw,inviteOutputIndex:1,inviteePublicKey:new Uint8Array(32),probationStartBlock:0,probationEndBlock:0 }], signatures:{}, protocolVersion:PROTOCOL_VERSION };
}
