...
describe('identity records in the block journal (Spec G phase B2)', () => {
  ...
  it('a put with no journal open records nothing', async () => {
    ...
    putIdentityRecord(uidBytes(), { lastActivityBlock: 1, lastDecayBlock: 0, likeCarry: 0n, invitedAtBlock: 0 });
  ...
  it('a first put is journaled with no `replaced`', async () => {
    ...
    const id = uidBytes();
    beginBlockJournal(1);
    putIdentityRecord(id, { lastActivityBlock: 10, lastDecayBlock: 0, likeCarry: 0n, invitedAtBlock: 0 });
    ...
    expect(m.record).toEqual({ lastActivityBlock: 10, lastDecayBlock: 0, likeCarry: 0n, invitedAtBlock: 0 });
  ...
  it('a put over an existing record is journaled with its `replaced` value', async () => {
    ...
    putIdentityRecord(id, { lastActivityBlock: 5, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 });

    beginBlockJournal(1);
    putIdentityRecord(id, { lastActivityBlock: 40, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 });
    const j = finishBlockJournal();

    const m = j.mutations[0] as RecordMutation;
    expect(m.record).toEqual({ lastActivityBlock: 40, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 });
    expect(m.replaced).toEqual({ lastActivityBlock: 5, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 });
  });
  ...
  it('deleteIdentityRecord never records, even with a journal open', async () => {
    ...
    const id = uidBytes();
    putIdentityRecord(id, { lastActivityBlock: 1, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });
  ...
  it('the journal round-trips a record mutation through CBOR', async () => {
    ...
    const id = uidBytes();
    putIdentityRecord(id, { lastActivityBlock: 3, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });

    beginBlockJournal(7);
    putIdentityRecord(id, { lastActivityBlock: 9, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });
    insertBlockJournal(finishBlockJournal());

    const loaded = getBlockJournal(7)!;
    const m = loaded.mutations[0] as RecordMutation;
    expect(m.kind).toBe('record');
    expect(m.record).toEqual({ lastActivityBlock: 9, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });
    expect(m.replaced).toEqual({ lastActivityBlock: 3, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });
  ...
  it('revert restores the exact prior record', async () => {
    ...
    const id = uidBytes();
    const pre: IdentityRecord = { lastActivityBlock: 5, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 };
  ...
  it('two puts to the same key in one block revert to the PRE-BLOCK value', async () => {
    ...
    const id = uidBytes();
    const preBlock: IdentityRecord = { lastActivityBlock: 5, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 };
    putIdentityRecord(id, preBlock);

    // The load-bearing case: activity bump then decay, at the same height.
    beginBlockJournal(1);
    putIdentityRecord(id, { lastActivityBlock: 40, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 });   // write 1
    putIdentityRecord(id, { lastActivityBlock: 40, lastDecayBlock: 40, likeCarry: 0n, invitedAtBlock: 0 });  // write 2
    const j = finishBlockJournal();
  ...
  it('two puts to a key that did not exist pre-block revert to absent', async () => {
    ...
    const id = uidBytes();
    beginBlockJournal(1);
    putIdentityRecord(id, { lastActivityBlock: 4, lastDecayBlock: 0, likeCarry: 0n, invitedAtBlock: 0 });
    putIdentityRecord(id, { lastActivityBlock: 4, lastDecayBlock: 4, likeCarry: 0n, invitedAtBlock: 0 });
    const j = finishBlockJournal();
  ...
  it('revert of interleaved writes to two identities restores both', async () => {
    ...
    const a = uidBytes();
    const b = uidBytes();
    const preA: IdentityRecord = { lastActivityBlock: 1, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 };
    putIdentityRecord(a, preA);
    // b has no pre-block record.

    beginBlockJournal(1);
    putIdentityRecord(a, { lastActivityBlock: 20, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });
    putIdentityRecord(b, { lastActivityBlock: 21, lastDecayBlock: 0, likeCarry: 0n, invitedAtBlock: 0 });
    putIdentityRecord(a, { lastActivityBlock: 20, lastDecayBlock: 20, likeCarry: 0n, invitedAtBlock: 0 });
    const j = finishBlockJournal();
  ...
  it('revertBlock restores the pre-block record for a twice-written key', async () => {
    ...
    beginBlockJournal(1);
    putIdentityRecord(id, { lastActivityBlock: 40, lastDecayBlock: 2, likeCarry: 0n, invitedAtBlock: 0 });
    putIdentityRecord(id, { lastActivityBlock: 40, lastDecayBlock: 40, likeCarry: 0n, invitedAtBlock: 0 });
    insertBlockJournal(finishBlockJournal());
  ...
  it('revertBlock restores the exact prior record for a single overwrite', async () => {
    ...
    const id = uidBytes();
    const pre: IdentityRecord = { lastActivityBlock: 7, lastDecayBlock: 3, likeCarry: 0n, invitedAtBlock: 0 };
    putIdentityRecord(id, pre);

    beginBlockJournal(1);
    putIdentityRecord(id, { lastActivityBlock: 99, lastDecayBlock: 3, likeCarry: 0n, invitedAtBlock: 0 });
    insertBlockJournal(finishBlockJournal());
  ...
  it('revertBlock leaves no record mutations journaled behind it', async () => {
    ...
    putIdentityRecord(id, { lastActivityBlock: 1, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });
    beginBlockJournal(1);
    putIdentityRecord(id, { lastActivityBlock: 2, lastDecayBlock: 1, likeCarry: 0n, invitedAtBlock: 0 });
    insertBlockJournal(finishBlockJournal());
