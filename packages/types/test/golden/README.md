...
| `boxes.json` | `canonicalBoxBytes` — box identity, one vector per box type, both roles of `like_accrual`, both states of `genesis_proof.payload`, and `emission` and `fee` at zero value AND zero height, which is the format's three-byte floor |
